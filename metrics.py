...
def circle_iou(cx1, cy1, r1, cx2, cy2, r2):
    """IoU analytique exact entre deux cercles."""
...
def bbox_to_circle(pt1, pt2):
    """
    Convertit 2 points labelme (bounding box du cercle) → (cx, cy, r).
...
def normalize_label(label):
    """
    Normalise n'importe quelle variante de label vers le format
    canonique du pipeline : '1 cent', '2 Euro', etc.
    """
    # Nettoyer : minuscules, sans espaces
    ...
    mapping = {
        ...
        "2euro":   "2 Euro",
        "2euros":  "2 Euro",
        # formats déjà corrects (avec espace)
        "1cent":   "1 cent",
    }
...
def compute_metrics(detections, ground_truths, iou_threshold=0.5):
    """
    Compare détections vs vérité terrain.

    detections    : liste de (cx, cy, r, label)   ← sorties du pipeline
    ground_truths : liste de (cx, cy, r, label)   ← depuis JSON annotation
    iou_threshold : seuil IoU pour valider une détection (défaut 0.5)

    Retourne un dict avec TP, FP, FN, précision, rappel, F1,
    label_accuracy et le détail de chaque match.
    """
    matched_gt  = set()
    tp_list, fp_list = [], []

    for i, (cx_d, cy_d, r_d, label_d) in enumerate(detections):
