...
from evaluate import run_pipeline, load_annotation
...
def test_dataset(test_dir, ann_dir,
    ...
    for fname in files:
        ...
        # Comparer avec annotation si disponible
        gt_total = 0.0
        if os.path.exists(ann_path):
            ...
            print(f"    Prédit {pred_total:.2f}€  |  Réel {gt_total:.2f}€"
                  f"  |  Erreur {err:.2f}€")
            print(f"    TP={m['TP']} FP={m['FP']} FN={m['FN']}"
                  f"  P={m['precision']:.2f} R={m['recall']:.2f}")
        else:
            print(f"    Prédit {pred_total:.2f}€  (pas d'annotation)")
    ...
    print(f"  RÉSUMÉ TEST")
    print(f"{'='*65}")
    print(f"  Pièces détectées : TP={all_tp} FP={all_fp} FN={all_fn}")
    print(f"  Précision        : {g_prec:.3f}")
    print(f"  Rappel           : {g_rec:.3f}")
    if errors:
        print(f"  Erreur monétaire moyenne : {sum(errors)/len(errors):.3f}€")
        print(f"  Erreur monétaire max     : {max(errors):.3f}€")
    print(f"  Total prédit     : {total_pred_eur:.2f}€")
    if total_gt_eur > 0:
        print(f"  Total réel       : {total_gt_eur:.2f}€")
...
def _save_test_image(img, detects, path):
    ...
    for cx, cy, r, lbl, d_mm, conf in detects:
        ...
        draw_label(out, f"{norm_lbl} {conf*100:.0f}%  {val:.2f}€",
