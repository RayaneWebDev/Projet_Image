...
from evaluation.metrics import compute_metrics, bbox_to_circle, normalize_label
...
def load_annotation(json_path, scale=1.0):
    """
    Charge les annotations LabelMe depuis un fichier JSON.
    Le rayon est multiplie par 2 car l'annotateur a clique sur le bord interieur.
    Retourne une liste de (cx, cy, r, label).
    """
...
def run_pipeline(image):
    """
    Execute segmentation + extraction features + classification sur une image.
    Retourne une liste de (cx, cy, r, label, d_mm, conf).
    """
...
def evaluate_dataset(val_dir, ann_dir, iou_threshold=IOU_THRESHOLD, save_dir=None):
    """
    Evalue le pipeline sur tout le dossier val_dir.
    Affiche les metriques par image et globales.
    Retourne True si la validation est reussie.
    """
    ...
    for fname in files:
        ...
        # Dimensions originales depuis le JSON pour le calcul du scale
        ...
        ok  = m["precision"] >= SUCCESS_PRECISION and m["recall"] >= SUCCESS_RECALL
        tag = "+" if ok else "x"

        print(f"\n  [{tag}] {fname}")
        print(f"       GT={len(gt)}  Det={len(dets)}"
...
def _print_metric(name, value, threshold):
    ...
    if threshold is not None:
        verdict = "  OK" if value >= threshold else "  X"
    print(f"  {name:<18}: {value:.3f}  {seuil}{verdict}")


def _save_annotated(img, detects, gt, path):
    """Sauvegarde l'image avec cercles predits (vert/orange/rouge) et GT (bleu)."""
    out = img.copy()

    for cx, cy, r, lbl in gt:
        cv2.circle(out, (int(cx), int(cy)), int(r), (255, 100, 0), 2)
        draw_label(out, normalize_label(lbl),
                   (int(cx - r), int(cy - r) - 12), color=(255, 100, 0))
    ...
    for cx, cy, r, lbl, d_mm, conf in detects:
        ...
        draw_label(out, f"{normalize_label(lbl)} {d_mm:.1f}mm {conf*100:.0f}%",
                   (int(cx - r), int(cy + r) + 14), color=color)
